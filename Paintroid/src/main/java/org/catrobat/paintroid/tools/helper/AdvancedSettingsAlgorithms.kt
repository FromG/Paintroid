...
class AdvancedSettingsAlgorithms {
    companion object {
                    ...
                    if (i >= 0) {
                        ...
                        when (i) {
                            0 -> {
                                ...
                                diffPointArray.add(PointF ((next.x - point.x) / 3, (next.y - point.y) / 3))
                            }
                            pointArray.size - 1 -> {
                                val prev: PointF = pointArray[i - 1]
                                diffPointArray.add(PointF((point.x - prev.x) / 3,(point.y - prev.y) / 3))
                            }
                            else -> {
                                val next: PointF = pointArray[i + 1]
                                val prev: PointF = pointArray[i - 1]
                                diffPointArray.add(PointF((next.x - prev.x) / 3,(next.y - prev.y) / 3))
                            }
                        }
                    }
                }
